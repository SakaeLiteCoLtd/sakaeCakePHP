...
//arrOrderEdisここまで
				...
				->order(["date_stock"=>"ASC"])->toArray();
					...
					for($k=0; $k<count($StockProducts); $k++){
						...
						if(isset($Product[0])){//'primary_p' => 1の場合
...
//arrSyoyouKeikakusここまで
	           ...
	           $LabelSetikkatsu2 = $this->LabelSetikkatsues->find()->where(['product_id2' => $KadouSeikeis[$k]['product_code'], 'kind_set_assemble' => 0])->toArray();
						 ...
						 if(isset($LabelSetikkatsu1[0])){//セット鳥の場合はもう片方も登録
...
/*
			echo "<pre>";
			print_r("OrderEdis".count($arrOrderEdis));
			print_r("StockProducts".count($arrStockProducts));
			print_r("AssembleProducts".count($arrAssembleProducts));
			print_r("SyoyouKeikakus".count($arrSyoyouKeikakus));
			print_r("Seisans".count($arrSeisans));
			echo "</pre>";
*/
/*
			$this->set([

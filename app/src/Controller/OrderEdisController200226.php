  ...
  //         $keys[array_search('15',$keys)]='place_deliver_name';
      ...
      try {//トランザクション4
          if ($this->OrderEdis->saveMany($orderEdis)) {//saveManyで一括登録
            ...
            for($n=0; $n<=10000; $n++){
              if(isset($arrDenpyou[$n])){//実験（OK）
                $OrderEdi = $this->OrderEdis->find()->where(['num_order' => $arrDenpyou[$n]['num_order']])->toArray();
        				$OrderEdi_id = $OrderEdi[0]->id;
                $arrDenpyou[$n] = array_merge($arrDenpyou[$n],array('conf_print'=>0));
                $arrDenpyou[$n] = array_merge($arrDenpyou[$n],array('delete_flag'=>$OrderEdi_id));
            ...
            if ($this->DenpyouDnps->saveMany($denpyouDnps)) {//saveManyで一括登録
              ...
              for($n=0; $n<=10000; $n++){
                if(isset($arrKannou[$n])){
                  ...
                  print_r(${"kannnou".$n});
                  echo "</pre>";
                    for($m=0; $m<=$n; $m++){//ホントはdate_deliver順に並べ替えたい➝bunnou,minoukannouの更新

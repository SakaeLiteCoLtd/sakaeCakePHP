...
*/
              ...
              $updater = "UPDATE order_dnp_kannous set updated_at = '".date('Y-m-d H:i:s')."', amount = $newamount,
               date_deliver = '".$newdate_deliver."'
                where product_id ='".$mikanproduct_code."' and num_order = '".$mikannum_order."'
                 and date_order = '".$mikandate_order."' and code = '".$mikanline_code."' and bunnou = '".$bunnnou."'
                  and date_deliver = '".$date_delivermoto."'";//もとのDBも更新
              $connection->execute($updater);
...
*/
      for ($k=0; $k<=$data["num"]; $k++){
        ...
        $ProductGaityu = $this->ProductGaityus->find()->where(['product_code' => $product_code, 'flag_denpyou' => 1,  'status' => 0])->toArray();

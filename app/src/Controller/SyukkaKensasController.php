...
//      $dirName = 'data_IM測定/P2133-93020_all/';//webroot内にアクセスされる
...
/*
                  			...
                  			echo "<br>";
*/
                        $file = readdir($child_dir);
  //                      $open_filename = $dirName.$folder."/".$file;
                        $open_filename = $dirName.$folder;
        								$fp = fopen("$open_filename", "r");//
                        $toDirCopy =  'data_IM測定/'.$folder;
      									$toCopyFile = "testCopyFile".$product_id;
                        rename($toCopyFile, 'backupData_IM測定/testCopyFile/' );//backupData_IM測定の中にフォルダを作る。
                        rename($open_filename, $toCopyFile);//フォルダ名を変更する。
    						...
    						if(substr($file, -4, 4) == '.csv' ){//csvファイルだけOPEN
    							...
    							if(substr($file, 0, 5) <> "sumi_" ){//sumi_でないファイルだけOPEN
    								...
    								for($i=0;$i<count($box);$i++){//kensahyou_headの数だけループ
    									...
    									$db->query("set names sjis");
    									$sql_change_file="select num from im_change_csvfile_num where kind_kensa = '".$folder."' and date = '".$today."'";
    									$rs_change_file=$db->query($sql_change_file);

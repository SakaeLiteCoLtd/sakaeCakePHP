...
$this->KouteiKensahyouHeads = TableRegistry::get('kouteiKensahyouHeads');//kensahyouHeadsテーブルを使う
$this->KouteiImKikakuTaious = TableRegistry::get('kouteiImKikakuTaious');//kensahyouHeadsテーブルを使う
?>
        <?php
...
<?php if(isset($ImKikakuex[0])): ?>
  ...
  <?php
    $KensaProduct = $this->KouteiImKikakuTaious->find()->where(['product_code' => $product_code ,'status' => '0'])->order(["version"=>"desc"])->toArray();
